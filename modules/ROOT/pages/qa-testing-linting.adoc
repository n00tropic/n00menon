...
* +scripts/trunk-upgrade-workspace.sh --check+ upgrades Trunk plugins and re-syncs definitions; run it when bumping lint/format tooling or after updating +.trunk/+ in Cortex (agents can call it through the +workspace.trunkUpgradeWorkspace+ capability).
