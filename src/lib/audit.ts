...
function loadSchema(root: string) {
  ...
  const schemaPath =
    ...
    path.resolve(root, "../../platform/n00-cortex/schemas/document-metadata.schema.json");
  ...
  if (!finalPath) {
    throw new Error(
      `Could not find document-metadata.schema.json. Searched locations: ${possiblePaths.join(", ")}`
    );
  }
